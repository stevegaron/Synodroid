...
class DSHandlerDSM31 implements DSHandler {
	 ...
	 * @param serverP
	 */
	public DSHandlerDSM31(SimpleSynoServer serverP, boolean debug) {
		server = serverP;
		DEBUG = debug;
	}
	...
	public void uploadUrl(Uri uriP, String uname, String pass) throws Exception {
		...
		if (server.isConnected()) {
			if (uriP.toString() != null) {
				...
				// The url_http's part
				if (uriP.toString().toLowerCase().startsWith("http:")) {
				...
				// The url_ftp's part
				if (uriP.toString().toLowerCase().startsWith("ftp:")) {
					builder.addPart(new Part("url_ftp").setContent(uriP.toString().getBytes()));
				} else {
					builder.addPart(new Part("url_ftp").setContent("".getBytes()));
				}
					
				if (uriP.toString().toLowerCase().startsWith("magnet:")) {
				...
				// Execute
				synchronized (server) {
					server.sendMultiPart(DM_URI_NEW, builder);
				}
			}
		}
	}
	...
	/* 
	 * REMOVED to use old link method...
	 * 
	public void upload(Uri uriP) throws Exception {
		// If we are logged on
		if (server.isConnected()) {
