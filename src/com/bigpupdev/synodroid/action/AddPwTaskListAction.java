 ...
 */
public class AddPwTaskListAction implements SynoAction {
	 ...
	 * @param outside_url
	 */
	public AddPwTaskListAction(List<Uri> outlines, String username, String password) {
		uriList = outlines;
		task = new Task();
		for (Uri uri: uriList){
			String temp = null;
	...
	public void execute(ResponseHandler handlerP, SynoServer serverP) throws Exception {
		if(serverP.getDsmVersion().smallerThen(DSMVersion.VERSION3_1)){
			...
			serverP.getDSMHandlerFactory().getDSHandler().uploadUrlList(uriList, uname, pass);
		}
	}
